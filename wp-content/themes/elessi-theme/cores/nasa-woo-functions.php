...
if (!function_exists('elessi_remove_action_woo')) :
    function elessi_remove_action_woo() {
        ...
        if (NASA_WISHLIST_NEW_VER) {
            add_filter('yith_wcwl_loop_positions', 'elessi_remove_default_wishlist_button');
        }
    }
endif;
...
if (!function_exists('elessi_add_to_cart_in_wishlist')) :
    function elessi_add_to_cart_in_wishlist() {
        ...
        $addToCart = apply_filters(
            ...
            sprintf(
                '<a href="%s" rel="nofollow" data-quantity="1" data-product_id="%s" data-product_sku="%s" class="button-in-wishlist small btn-from-wishlist %s product_type_%s add-to-cart-grid wishlist-fragment" data-type="%s" title="%s">' .
        ...
        if ($product_type === 'variable') {
            $addToCart .= sprintf('<a href="javascript:void(0);" class="quick-view nasa-view-from-wishlist hidden-tag" data-prod="%s" data-from_wishlist="1">%s</a>', $productId, esc_html__('Quick View', 'elessi-theme'));
        }
        ...
        $result .= apply_filters(
            ...
            sprintf(
                '<div class="add-to-cart-btn btn-link add-to-cart-icon">' .
                    '<a href="%s" rel="nofollow" data-quantity="1" data-product_id="%s" data-product_sku="%s" class="%s product_type_%s" title="%s"' . $attributes . '>' .
                        '<span class="add_to_cart_text">%s</span>' .
                ...
                esc_attr($product->get_sku()), //product sku
                esc_attr($class_button), //class name
                esc_attr($product_type), //product type
                esc_attr($title),
                $title
            ),
...
if (!function_exists('elessi_get_product_meta_value')):
    ...
    function elessi_get_product_meta_value($post_id, $field_id = null) {
        global $nasa_meta_values;
        
        if (!isset($nasa_meta_values)) {
            $nasa_meta_values = get_post_meta($post_id, 'wc_productdata_options', true);
            $GLOBALS['nasa_meta_values'] = $nasa_meta_values;
        }
        
        if (isset($nasa_meta_values[0]) && $field_id) {
            return isset($nasa_meta_values[0][$field_id]) ? $nasa_meta_values[0][$field_id] : '';
        }

        return isset($nasa_meta_values[0]) ? $nasa_meta_values[0] : $nasa_meta_values;
    }
...
if (!function_exists('elessi_add_wishlist_button')) :
    function elessi_add_wishlist_button() {
        if (NASA_WISHLIST_ENABLE) {
            ...
            ?>
            <a href="javascript:void(0);" class="btn-wishlist btn-link wishlist-icon tip-top" data-prod="<?php echo (int) $productId; ?>" data-prod_type="<?php echo esc_attr($productType); ?>" data-original-product-id="<?php echo (int) $productId; ?>" data-tip="<?php esc_attr_e('Wishlist', 'elessi-theme'); ?>" title="<?php esc_attr_e('Wishlist', 'elessi-theme'); ?>">
                <i class="nasa-icon icon-nasa-like"></i>
...
if (!function_exists('elessi_loop_product_content_thumbnail')) :
    function elessi_loop_product_content_thumbnail() {
         ...
         */
        if (
            !in_array($nasa_animated_products, array('', 'hover-zoom')) && 
        ...
        if (!$attachment_ids && $nasa_animated_products !== 'hover-zoom') {
...
if (!function_exists('elessi_nasa_change_view')) :
    function elessi_nasa_change_view($nasa_change_view = true, $typeShow = 'grid-4', $nasa_sidebar = 'no') {
        ...
        $class_wrap = 'filter-tabs';
        $class_wrap .= $col_2 || $col_6 ? ' nasa-show-number' : '';
        ?>
 ...
 * For New version Yith Wishlist 3.0 or Higher
 */
if (!function_exists('elessi_remove_default_wishlist_button')) :
    function elessi_remove_default_wishlist_button($positions) {
        $positions = array();
